...
log_filename = os.path.join("logs/batch_processing", f"{timestamp}.log")
...
def main():
    ...
    parser.add_argument("-uc", "--use_case_name", type=str, help="Name of the test case", required=True)
    parser.add_argument("-c", "--configuration", type=str,  help="Configuration", required=True)
    parser.add_argument("-m", "--model", type=str, help="Model", default="llama3.3", required=True)
    ...
    prompt_template_path = f"./prompts/batch_processing/{configuration}/{model_name_short}"
    ...
    output_path = f"./output/batch_processing/{configuration}/" + model_name_short
    ...
    prompt = prompt_builder.build_prompt(None, test_cases_str, dependent_uc_code, pom_content)
    ...
    javascriptCodeValidator = JavascriptCodeValidator(use_case_name, None, response.response)
    ...
    with open(os.path.join(folder_abs_path, f"{use_case_name}.json"), "w") as f:
